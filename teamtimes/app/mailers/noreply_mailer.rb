class NoreplyMailer < ApplicationMailer
	default from: 'rijusikdar@gmail.com'

	# include ApplicationMailer
	# def mailtimes(team) #mails for one team only
	# 	emails = team.teamemail.split(',')
	# 	emails.each do |e|
	# 		mail(to: e, subject: "Your Upcoming Games")
	# 	end
	# end
	...
	def mailtimes(user)
	  	@user = user.email
		###import
  		@teams = Team.select([:name]).where(:belongs_to => user.email)
  		@times = Location.select([:name, :date]).where(:belongs_to => user.email)
  		@emails = Team.select([:teamemail]).where(:belongs_to => user.email)
  		...
  		@emails.each do |team|
  			if team.teamemail != nil
  				emailstosend <<team.teamemail.split(",")
  			end
		...
		###make into words
	  	...
	  	mail(to: emailstosend, subject:'Game Time - Match List')
